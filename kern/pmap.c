...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!new_ptep) return NULL;
	new_ptep->pp_ref ++;
	*pde = page2pa(new_ptep) | PTE_P | PTE_U | PTE_W;
...
{
	// Fill this function in
	int i, num_pg, create = 1;
	pte_t *pte;
	
	num_pg = size / PGSIZE;
	for (i = 0; i < num_pg; i++) {
		pte = pgdir_walk(pgdir, (void*)va, create);
		if (!pte) {
			panic("out of memory!\n");
		}
		*pte = pa | PTE_P | perm;
